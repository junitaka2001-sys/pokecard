...
class StorageManager {
  ...
  // ポイント消費（手動/自作消費）
  async consumePoints(amount = 1, reason = 'ポイント使用') {
    const safeAmount = parseInt(amount, 10);
    if (isNaN(safeAmount) || safeAmount <= 0) {
      return { success: false, message: '無効なポイント数です' };
    }

    const current = await this.getStamps();
    if (current < safeAmount) {
      return { success: false, message: 'ポイントが足りません', current };
    }

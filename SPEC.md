...
| 3.2.0 | スタンプポイント化・10pt周回制導入：上限制限解除、ホーム画面に「ポイントを使う」セクション追加、2タブ構成（ホーム/抽選）へ変更 |
| 3.2.1 | 周回計算の修正：10pt達成時に即座に完成カード1枚バッジを表示し、11pt目で2枚目のカードがスムーズに開始されるよう計算式を修正 |
| 3.3.0 | 使い方チュートリアル機能追加：初回起動時の使い方モーダル表示（ホーム/抽選の説明、スタンプ1個=500円相当表記）、ホーム画面に「🔰 使い方」再表示ボタンを追加 |
| 3.3.1 | パフォーマンス超高速化・バグ修正：不要な直列Supabase通信を全廃しPromise.all並列処理化、HTML入れ子エラー修正による「🔰 使い方」ボタン即時表示対応、自動モーダル廃止（ボタン押下時のみ表示） |
| 3.3.2 | 使い方ガイド文面のブラッシュアップ：見出し・絵文字・太字入りのポップ＆直感的な文面へ全面更新 |
| 3.4.0 | ロックマンエグゼ風クリアスター機能追加：スタンプカード右上エリアに周回完了ごとにカラフルな星（8色ループ、SVGグラデーション＋popInアニメーション）を表示 |

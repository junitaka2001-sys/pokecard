...
**最終更新: 2026-09-23 (Ver 3.4.0 ロックマンエグゼ風クリアスター機能追加) / 更新者: Antigravity**
...
|------|------|
| バージョン | **3.4.0** |
| 実装フェーズ | **ロックマンエグゼ風クリアスター機能追加（周回完了ごとにカラフルな星を表示）** |
| 動作確認 | 実機未確認（iPhone Safari実機確認必要） |
| Gitコミット | ローカルコミット完了 (`main` ブランチ) |
| デプロイ | GitHub Pages へ push 済み |
...
- [x] **使い方チュートリアルガイド** (アプリ初回起動時に全画面ガイド表示、スタンプ1個=500円表記、ホーム画面に「🔰 使い方」ボタン設置)
...
**ロックマンエグゼ風クリアスター機能（v3.4.0）の実装が完了。git push 済み。**
...
### 残タスク
- はるかの iPhone Safari での実機確認（クリアスター表示・PWAインストール含む）
- Supabase SQL Editor で `supabase/phase3_remove_stamp_limit.sql` を実行（10pt超の加算が動かない場合）
